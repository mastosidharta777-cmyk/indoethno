...
export function ExperienceRequestForm({ selected = '', interestOnly = false }: { selected?: string; interestOnly?: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState('success');
      setMessage('Inquiry received. This is not a booking confirmation; IndoEthno will only proceed after feasibility and availability are checked.');
  ...
  return (
    <form className="form requestForm" onSubmit={submit}>
      ...
      {interestOnly && <div className="noticeCard"><strong>Interest only</strong><p>This artist is not currently presented as available by request. Submitting this measures group demand without promising a performance.</p></div>}
      ...
      <small className="formNote">Inquiry only. No artist, price, date or performance is confirmed by submitting this form.</small>
    </form>
  );
}
